...
def topk_eval(device, model, graph, user_list, train_record, test_record, item_set, i_nodes, k_list, batch_size):
    ...
    for user in tqdm(user_list):
        ...
        while start + batch_size <= len(test_item_list):
            ...
            # print(i, i.size())
            # scores = F.sigmod(model(u, i, graph)).tolist()
            scores = model(u, i, graph).tolist()
            # logging.info('topk.user.scores:' + str(scores))
        ...
        for k in k_list:
            ...
            # logging.info('k=' + str(k) + ' item_sorted[:k]:'+ str(item_sorted[:k]))
            # logging.info('k=' + str(k) + ' test_record[' + str(user) +']:'+ str(test_record[user]))
            # logging.info('k=' + str(k) + ' user:'+ str(user) + ' hit_num:' + str(hit_num) + '\n')
            # precision
